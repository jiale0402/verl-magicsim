...
class AsyncRolloutRequest(BaseModel):
    ...
    def add_assistant_message(
        ...
        if format in self.format_config:
            ...
            prefix_token_ids = tokenizer.tokenizer.encode(prefix_msg, add_special_tokens=False)
            suffix_msg = self.format_config[format]["assistant_suffix_msg"]
            suffix_token_ids = tokenizer.tokenizer.encode(suffix_msg, add_special_tokens=False)
            if tool_calls is not None:
                content = content_with_tool_calls.split(f"{prefix_msg}")[-1].split(f"{suffix_msg}")[0]
            content_token_ids = tokenizer.tokenizer.encode(content[0]["text"], add_special_tokens=False)
            if self.input_ids[-len(prefix_token_ids) :] == prefix_token_ids:
                ...
                raise ValueError(
                    ...
                    {tokenizer.tokenizer.decode(self.input_ids[-max_len:])},
                    {tokenizer.tokenizer.decode(self.input_ids)}, {self.messages}"""
                )
